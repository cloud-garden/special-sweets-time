...
public class OnigiriSalesByTaste {
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
		...
		String outputpath = "out/onigiriCount/kadai7/";     // ★MRの出力先
		if (args.length > 0) {
			inputpath = args[0];
		}
	...
	public static class MyMapper extends Mapper<LongWritable, Text, CSKV, CSKV> {
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			if (csv[PosUtils.ITEM_NAME].matches(".*(梅|(うめ)).*")) {
				taste = "うめ";
			}else if(csv[PosUtils.ITEM_NAME].matches(".*(鮭|(さけ)).*")) {
				taste = "さけ";
			}else{
				taste = csv[PosUtils.ITEM_NAME];
			}
			...
			context.write(new CSKV(taste), new CSKV(count));
		}
	}
