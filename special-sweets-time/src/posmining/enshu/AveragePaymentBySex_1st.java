...
public class AveragePaymentBySex_1st {
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
		...
		String outputpath = "out/onigiriCount/kadai8/";     // ★MRの出力先
		if (args.length > 0) {
			inputpath = args[0];
		}
	...
	public static class MyMapper extends Mapper<LongWritable, Text, CSKV, CSKV> {
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			String recId = csv[PosUtils.RECEIPT_ID];
			String sex = null;
			if(csv[PosUtils.BUYER_SEX].equals("1")){
				sex = "male";
			}else{
				sex = "female";
			}

			 int count = new CSKV(csv[PosUtils.ITEM_COUNT]).toInt();
			 int price = new CSKV(csv[PosUtils.ITEM_PRICE]).toInt();
			 int sum = count * price;

			context.write(new CSKV(sex +"_" +recId), new CSKV(sum));
		}
	}
	...
	public static class MyReducer extends Reducer<CSKV, CSKV, CSKV, CSKV> {
		protected void reduce(CSKV key, Iterable<CSKV> values, Context context) throws IOException, InterruptedException {
			...
			// emit
			String myKey = null;
			if(key.toString().startsWith("male")){
				myKey = "male";
			}else{
				myKey = "female";
			}
			context.write(new CSKV(myKey), new CSKV(sum));
		}
	}
}
