...
public class SweetsSumPrice {
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
		...
		job.setJarByClass(SweetsSumPrice.class);       // ★このファイルのメインクラスの名前
		...
		String outputpath = "out/okashige/sweets/sweetsSumPrice";     // ★MRの出力先
		if (args.length > 0) {
			inputpath = args[0];
		}
	...
	public static class MyMapper extends Mapper<LongWritable, Text, CSKV, CSKV> {
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			//keyとなるレシート番号と店舗立地を取得を取得
			String receiptId = csv[PosUtils.RECEIPT_ID] +"\t"+ csv[PosUtils.LOCATION];

			// emitする （emitデータはCSKVオブジェクトに変換すること）
			context.write(new CSKV(receiptId), new CSKV(price));
		}
	}
