package posmining.okashige.SweetsSumPrice;
...
public class SweetSumTotal {
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
		...
		job.setJarByClass(SweetSumTotal.class);       // ★このファイルのメインクラスの名前
		...
		String outputpath = "out/okashige/sweets/sweetsSumTotal";     // ★MRの出力先
		if (args.length > 0) {
			inputpath = args[0];
		}
	...
	public static class MyMapper extends Mapper<LongWritable, Text, CSKV, CSKV> {
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			//valueとなる合計購入金額を取得
			String total = csv[PosUtils.ITEM_TOTAL_PRICE];

			//keyとなるレシートID,店舗立地を取得
			String recipiId = csv[PosUtils.RECEIPT_ID] +"\t"+ csv[PosUtils.LOCATION];

			// emitする （emitデータはCSKVオブジェクトに変換すること）
			context.write(new CSKV(recipiId), new CSKV(total));
		}
	}
